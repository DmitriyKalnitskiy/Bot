...
async def sendWelcome(message: types.Message):
    await message.reply("\nДля того, чтобы пользоваться ботом введите команду /motivation."
                        "\nЧтобы просмотреть возможности бота введи команду /info.")

@dp.message_handler(commands=['motivation'])
async def ask_for_motivation(message: types.Message):
    await MotivationState.sending.set()
    await message.reply("Отправьте команду /photo, чтобы получить мотивационное изображение.")

@dp.message_handler(commands=['photo'], state=MotivationState.sending)
